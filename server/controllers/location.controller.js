...
const { calculateDistance, convertRange, normalizeScore } = require('../utils/location.utils');
...
const getPaginatedLocations = asyncHandler(async (req, res) => {
  try {
		...
		logger.log('error', 'Location Controller getPaginatedLocations error: ', new Error(error));
		res.status(500).json({ message: 'Internal Server Error' });
	}
});
...
const searchLocations = asyncHandler(async (req, res) => {
	...
	try {
		...
		const searchResults = locations.map(location => {
			const distance = calculateDistance(parseFloat(latitude), parseFloat(longitude), location.latitude, location.longitude);
			return {
		...
		if(searchResults.length > 1) {
			...
			const maxDistance = parseInt(searchResults[searchResults.length - 1].score);
			const minValue = minDistance - (minDistance * 0.35);
			const maxValue = maxDistance + (maxDistance * 0.15);
			const scoredResults = searchResults.map(result => {
				const newScore = normalizeScore(result.score, minValue, maxValue).toFixed(2);
		...
		logger.log('error', 'Location Controller searchLocations error: ', new Error(error));
		res.status(500).json({ message: 'Internal Server Error' });
	}
});
