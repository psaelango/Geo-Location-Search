      ...
      if (nearByLocations && nearByLocations.length > 0) {
        ...
        for (let i = 0; i < nearByLocations.length; i++) {
          ...
          new MapboxGl.Marker(elem)
            ...
            .setPopup(
              new MapboxGl.Popup({ offset: 25 }).setHTML(`<pre>${name}</pre>`),
            )
            .addTo(map);
  ...
  return (
    <div>
      <Alert key="primary" variant="primary">
        Note: This page displays entire data using server pagination. Click on
        the row to view nearby locations around that lat lng.
      </Alert>
