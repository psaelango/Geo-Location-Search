...
import { getAllLocations, getLocationsByScore } from "../store/location/locationService";
  ...
  const initMap = async () => {
		// try {
			...
			const data = locations.map(location => {
				const { street, city, zip_code, county, country, latitude, longitude } = location;
				return {
					'type': 'Feature',
					'properties': {
			...
			geoCoder.on('result', async function(e) {
				if (e.result.center && e.result.center.length === 2) {
					...
					const scoredLocations = await getLocationsByScore(user, `?q=${text}&latitude=${lat}&longitude=${lng}`)
  ...
  return (
    <div>
      <Alert key='primary' variant='primary'>
        Note: This page initially loads entire data for map input search bar. Second time it uses cached data from server.
        You can differentiate our data from 🌲 emoji on the search bar. 
      </Alert>
        ...
        <div style={{margin: '20px', flex: 1}}>
					{
							geoCoderStatus === 'loading' ? 
								<Spinner animation="border" role="status">
									<span className="visually-hidden">Loading entire geo-location data ....</span>
								</Spinner>
							:
          			<TableDisplay data={locations} fields={tableFields} highlight={selectedMarker}/>
					}
        </div>
